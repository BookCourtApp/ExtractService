...
public class LiveLibUserInfoExtractor : IExtractor<IDocument, IEnumerable<UserPreference>>
{
     public async Task<IDocument> GetRawDataAsync(ResourceInfo resource)
    {
     ...
     {
         // var isEmpty = data.GetElementsByClassName("with-pad")[0].TextContent.ToLower()
         //     .Contains("этот список пока пуст.");
         // if (isEmpty)
         //     return null;
         ...
         try
         {
             var login = data.GetElementsByClassName("header-profile-login")[0].TextContent;
             var divid = data.GetElementById("user-objects");
             var booklist = data.GetElementById("booklist").Children;
             if (booklist.Length < 0)
                 return null;
             string curData = "";
             var preftype = data.GetElementsByClassName("active")[0].TextContent.Split(' ')[0];
             foreach (var item in booklist)
             {
                 switch (item.ClassName)
                 {
                     ...
                     case "book-item-manage":
                         ...
                         break;
                     default:
                         Console.WriteLine($"КАКОЙ ТО НОВЫЙ ЭЛЕМЕНТ НА СТРАНИЦЕ, {data.Title}, {item.ClassName}");
                         break;
                 }
             }
         }
